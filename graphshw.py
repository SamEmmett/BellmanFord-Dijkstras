...
#########################################################################
    ...
    def bellmanFord(self, s) :
        ...
        #Initialize distance from source Vertex
        dist = [math.inf] * self.numVertices()
        dist[s] = 0
        parent = [None] * self.numVertices()

        #Relax all edges |V|-1 times
        for i in range(self.numVertices()-2):
            for (u, v), w in self.getEdgeList(True):
                if dist[u] + w < dist[v]:
                    dist[v] = dist[u] + w
                    parent[v] = u
                if dist[v] + w < dist[u]:
    ...
    def dijkstra(self,s) :
        ...
        #return 3-tuple (vertexID, distance/weight, vertex parent)
        """Weight = priority?"""
        ...
        Q = PQInts(self.numVertices())
        #Q.insert(0, dist[s])
        for u in range(self.numVertices()): #Populates the QUEUE
            Q.insert(u, dist[u])
