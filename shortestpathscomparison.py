...
def timeShortestPathAlgs() :
    ...
    #        data shouldn't be affected much by source vertex.
    numRuns = 10
    vertData = [16, 64, 256, 16, 64, 256]
    edgeData = [120, 2016, 32640, 32, 128, 512]

    print('{:^7}'.format("Vertices"), "|", '{:^7}'.format("Edges"), "|",  '{:^12}'.format("BellmanFord"), "|", '{:^12}'.format("Dijkstra"), "|")
    i = 0
    while i != len(vertData):
        graph = randomWeightedGraph(vertData[i], edgeData[i], 1, 10)
        ...
        print('{:^7}'.format(vertData[i]), " | ",'{: ^5}'.format(edgeData[i]), '{: ^3}'.format("|"), '{0:.8f}'.format(t1/numRuns) , " | ", '{0:.8f}'.format(t2/numRuns), " |")
        i = i + 1
